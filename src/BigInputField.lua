...
local BigInputField = classes.class()

function BigInputField:init(x, y, w, h, label, bg, fg, textBg, lines)
  self.x = x
  self.y = y
  self.w = w
  self.h = h
...
function BigInputField:hide()
  gpu.setBackground(BACKGROUND)
  gpu.fill(self.x, self.y, self.w, self.h, " ")
end

function BigInputField:readInput()
  local entries = self:showLines()
  local editing = true
  while editing do
    ...
    if x ~= nil and y ~= nil and x >= self.x + 1 and x < (self.x + self.w - 2) and y >= self.y + 4 and y < (self.y + self.h - 2) then
      ...
      if string.len(newEntry) == 36 or string.len(newEntry) == 0 then
        self.lines[entryNum + 1] = newEntry
      end
      entries = self:showLines()
    end
...
function BigInputField:readLine(lineNum)
  local text = ""
  local typing = true
...
function BigInputField:showLines()
  local entries = 0
  gpu.setBackground(self.textBg)
